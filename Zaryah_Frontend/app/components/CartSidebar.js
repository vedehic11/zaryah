        ...
        {carts && carts.length > 0 ? (
          <div className="space-y-6">
            {carts.map((cart, cartIndex) => (
              <div key={cart.id} className="border-b border-gray-200 pb-4 last:border-0">
                {/* Cart Header (Seller Name) */}
                {carts.length > 1 && (
                  <div className="mb-3 flex items-center justify-between bg-primary-50 p-3 rounded-lg">
                    <h3 className="font-semibold text-primary-900">
                      Cart {cartIndex + 1}: {cart.seller_name}
                    </h3>
          ...
          {carts.length > 1 && (
            <div className="mb-3 p-3 bg-blue-50 border border-blue-200 rounded-lg">
              <p className="text-xs text-blue-900">
                ℹ️ You have {carts.length} separate carts from different sellers. You can checkout each cart separately.
              </p>
            </div>
          )}
