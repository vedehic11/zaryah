...
export const HomePage = () => {
  ...
  // Use window scroll instead of container scroll to avoid positioning issues
  const { scrollY } = useScroll()
  // Get the top offset of the hero section - use state to avoid hydration mismatch
  const [heroHeight, setHeroHeight] = useState(800)
  
  useEffect(() => {
    // Set height on client side only
